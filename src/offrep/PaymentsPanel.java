...
public class PaymentsPanel extends javax.swing.JPanel {
    ...
    private void save(){
         try{
             if(comboClient.getSelectedItem().toString().equals("Select Client")){
                ...
                String sql = "INSERT INTO clients_paytable(payment_id,client_id,amount,s,user_id,payment_date,company_id)VALUES(?,?,?,?,?,?,?)";
    ...
    public ArrayList<SearchPayment> Listpayments(String ValToSearch){
        ...
        try{
            String searchQuery = "SELECT * FROM clients_paytable,clientstable WHERE clients_paytable.client_id = clientstable.clientid AND "
                    ...
                    + " LIKE'%"+ValToSearch+"%' AND clients_paytable.s = '1' AND clients_paytable.company_id = '"+loggedcompanyid+"'";
    ...
    private void selectedrow(){
        try{
            manage.showdialog(DialogPayments, jPanel1, 716, 259);
            fillcombo();
            ...
            String sql = "SELECT * FROM clients_paytable,clientstable WHERE clients_paytable.client_id = clientstable.clientid AND "
                    + "clients_paytable.payment_id = '"+table_click+"' AND clients_paytable.company_id = '"+loggedcompanyid+"'";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
                if(rs.next()){
                    ...
                    comboClient.setSelectedItem(rs.getString("clientstable.name"));
                    clientid = rs.getInt("clients_paytable.client_id");
                    lblClient.setText(rs.getString("clientstable.email"));
                    txtAmount.setText(rs.getString("amount"));
                }
        }catch(Exception e){
            System.out.println(e+" paymentspanel.selectedrow");
        }finally{
            try{
                rs.close();
                pst.close();
    ...
    private void delete(){
        ...
        if (response == JOptionPane.NO_OPTION){
            ...
            String sql = "UPDATE clients_paytable SET s = '0' WHERE payment_id = '"+txtPaymentNo.getText()+"' AND s = '1' AND company_id = '"+loggedcompanyid+"'";
            manage.update(sql);
            String query = "UPDATE paymentstable SET s = '0' WHERE details = '"+txtPaymentNo.getText()+"' AND s = '1' AND company_id = '"+loggedcompanyid+"'";
            manage.update(query);
            setbalance_clients();
            reset();
            loadtable();
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/offrep/icons/payments.png"))); // NOI18N
        ...
        jLabel5.setIcon(new javax.swing.ImageIcon(getClass().getResource("/offrep/icons/search.png"))); // NOI18N
